...
using AntiPhisher.Application.MyMapper;
using AntiPhisher.Application.Services;
using AntiPhisher.Application.Validation;
using AntiPhisher.Domain;
using AntiPhisher.Infrastructure;
...
// Bật Swagger luôn luôn
app.UseSwagger();
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "AntiPhisher API V1");
...
using (var scope = app.Services.CreateScope())
{
    ...
    catch (Exception ex) { scope.ServiceProvider.GetRequiredService<ILogger<Program>>().LogError(ex, "Seeding failed."); }
}
